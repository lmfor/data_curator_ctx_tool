...
def call_contextual_agent(metadata: Dict[str, Any]) -> Dict[Any, Any]: # type: ignore
    ...
    for page in metadata:
        ...
        page_url = page['url'] if 'url' in page else '' #type: ignore
        # page_breadcrumbs = page['breadcrumbs'] if 'breadcrumbs'
        ...
        if relevance_score is not None and currency_score is not None:
            ...
            if(relevance_score > 0.80 and currency_score == 1.0): # play around with these values (mostly relevance score)
                # LOGIC FOR DB
                print(f"""
